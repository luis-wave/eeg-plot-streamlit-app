...
def eeg_visualization_dashboard():
    ...
    if 'mw_object' in st.session_state and st.session_state.mw_object:
        ...
        with st.container() as row1:
            ...
            montage_options_dict = {
                "Linked-Ears": "a1a2",
                "Centroid": "cz",
                "Bipolar-Transverse": "bpt",
                "Temporal-Central Parasagittal": "tcp",
                "Average": "avg",
                "Recorded Infinite Rest": "ref",
            }
            curr_montage = st.selectbox(
                "Montage",
                montage_options_dict, # ["a1a2", "cz", "bpt", "tcp", "avg", "ref"],
            ...
            if (st.session_state.get("aev_aea_montage", None) != montage_options_dict[curr_montage]) or (st.session_state.get("fname", None) != st.session_state.get("aev_aea_fname", None)):            
                # 
                if (st.session_state.get("fname", None) != st.session_state.get("aev_aea_fname", None)):
                    get_data(mw_object, montage_options_dict[curr_montage], normalize_df=True) # df = waev.normalize_dataframe(df)
                ...
                st.session_state.aev_aea_montage = montage_options_dict[curr_montage]


            # Create plot
            # fig = create_plotly_figure(df, sfreq, channels, curr_montage)
            fig = draw_eeg_graph(
                st.session_state.aev_montage_data[montage_options_dict[curr_montage]].get("df", None), 
                st.session_state.aev_montage_data[montage_options_dict[curr_montage]].get("raw", None).info['sfreq'],
                st.session_state.aev_montage_data[montage_options_dict[curr_montage]].get("channels", None), 
                montage_options_dict[curr_montage],
            )
